...
const CreatePostAlternativeFirstForm = ({
	...
	const handleNext = () => {
		...
		if (data.img.length === 0) return errorHandler('You must provide at list one photo');
		setDisplay(true);
	};
		...
		<>
			<form className='h-80 capitalize overflow-auto flex flex-col gap-2'>
				<div className='flex flex-col w-full gap-2'>
					{/* post title */}
					<input type='text' name='title' onChange={handleOnChange} placeholder='Post title' value={data?.title} />
					...
					{/* term options */}
					<select onChange={handleOnChange} value={data?.range} className='m-0 capitalize' name='range'>
				...
				{/* upload post */}
				{!images.length > 0 ? (
					<div className='flex flex-col w-full' onClick={() => cloudinaryModal.open()}>
						<div className='cursor-pointer text-gray-500 flex flex-col items-center justify-center bg-white w-full rounded-xl p-3 h-full border border-gray-200'>
							...
							{images && images.length > 0 ? <h2 className='text-gray-900 top-4'>Upload More</h2> : null}
